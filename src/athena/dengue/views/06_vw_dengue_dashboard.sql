CREATE OR REPLACE VIEW "baip_dev_gold"."vw_dengue_dashboard" AS
SELECT
    CAST(
        date_parse(
            CONCAT(
                CAST(notification_year AS varchar),
                '-',
                notification_month,
                '-01'
            ),
...
FROM "AwsDataCatalog"."baip_dev_gold"."vw_dengue_cases_enriched"
GROUP BY
    notification_year,
    notification_month,
    disease_code,
