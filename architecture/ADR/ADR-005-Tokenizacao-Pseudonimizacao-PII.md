...
O BAIP (Brazil Arbovirus Intelligence Platform) possui um fluxo de eventos hospitalares simulados que pode conter identificadores pessoais, como CPF ou identificadores equivalentes.
...
* O processo de geração do token deve normalizar o identificador de entrada antes da tokenização, removendo pontuação, espaços e variações de formato.
...
Essa decisão também melhora a aderência aos princípios de minimização, segurança, prevenção e privacidade desde a concepção. Para o MVP, a abordagem demonstra boas práticas mesmo com dados simulados. Para um cenário real, ela cria uma base técnica mais segura para evolução regulatória e operacional.
